...
func TestQuerySqlComplex(t *testing.T) {
	...
	script := "SELECT id, name, email, balance, rating, birth_day, married, created_at FROM customer"
	rows, err := db.QueryContext(ctx, script)
	if err != nil {
		panic(err)
	}
...
func TestPrepareStatement(t *testing.T) {
	...
	for i := 0; i < 10; i++ {
		email := "fajrul" + strconv.Itoa(i) + "@gmail.com"
...
func TestTransaction(t *testing.T) {
	...
	script := "INSERT INTO comment(email, comment) VALUES(?, ?)"
	for i := 0; i < 10; i++ {
		email := "aslimdd" + strconv.Itoa(i) + "@gmail.com"
		comment := "komentar ke " + strconv.Itoa(i)
